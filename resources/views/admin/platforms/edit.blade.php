...
@section('main-content')
    ...
    <div class="form-style-5">
        <form action="{{ url('/platforms/' . $platform->id) }}" method="post">
            {{ method_field('PATCH') }}
            @csrf
            <fieldset>
                <legend><span class="number"><i class="fas fa-plus"></i></span> Edit Platform ({{ $platform->name }})</legend>
            </fieldset>
            <fieldset>
                @include ('admin.platforms.form', ['formMode' => 'edit'])
            </fieldset>
        </form>
    </div>
@endsection
