...
@section('main-content')
    ...
    <div class="form-style-5">
        <form action="{{ url('/employees/' . $employee->unique_key) }}" method="post" enctype="multipart/form-data">
            ...
            @csrf
            <fieldset>
                <legend><span class="number"><i class="fas fa-plus"></i></span> Edit Employee ({{ $employee->full_name }})</legend>
            </fieldset>
            <fieldset>
...
<script src="https://cdnjs.cloudflare.com/ajax/libs/bootstrap-datepicker/1.3.0/js/bootstrap-datepicker.js"></script>
    <script>
        $(document).ready(function(){
            ...
            var count = {{ $employee->certificates->count() }}+1;
            ...
            $(document).on('click', '.delete_certificate_button', function(){
                ...
                let certificate_id = $('.certificate_id_'+delRowAttr).val();
                if(certificate_id){
                    $.ajax({
                        type: 'POST',
